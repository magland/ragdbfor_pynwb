...
## Installation
Install NeuroConv with Thor TIFF support:
```bash
pip install "neuroconv[thor]"
```

## Usage Example
Convert Thor TIFF imaging data to NWB format:
...
**Note:** The `ThorImagingInterface` is specifically designed for imaging data acquired using ThorImageLS software and exported to TIFF format. If data was acquired with a Thor microscope but not with the ThorImageLS software, this interface may not work correctly.